...
conf.registerGlobalValue(SpiffyTitles, 'maxRetries',
    registry.Integer(3, _("""Maximum Retries prior to Failure""")))
...
conf.registerGlobalValue(SpiffyTitles, 'urlRegularExpression',
    ...
    r"[^\s]+\.[^\s]{2,})", _("""The regular expression used to match URLs""")))
...
conf.registerGlobalValue(SpiffyTitles, 'channelWhitelist',
    registry.CommaSeparatedListOfStrings([], _("""Only show titles on these
    channels, or all if empty""")))

# Channel blacklist
conf.registerGlobalValue(SpiffyTitles, 'channelBlacklist',
    registry.CommaSeparatedListOfStrings([], _("""Never show titles on these
    channels""")))
...
conf.registerGlobalValue(SpiffyTitles, 'youtubeDeveloperKey',
    registry.String("", _("""Youtube Developer Key - required for Youtube
    handler.""")))
